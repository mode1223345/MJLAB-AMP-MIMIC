...
def _resolve_callable(path: str):
  """Import ``module.path:func_name`` or ``module.path.func_name``."""
  if ":" in path:
...
class RandomNetworkDistillation:
  """Placeholder — RND is not enabled for the Labubu AMP port."""

  def __init__(self, *args, **kwargs):
    raise NotImplementedError("RND is not ported for mjlab AMP yet.")
...
class AMPHIMPPO:
  ...
  def __init__(
    ...
    if symmetry_cfg is not None:
      ...
      if isinstance(symmetry_cfg["data_augmentation_func"], str):
        symmetry_cfg["data_augmentation_func"] = _resolve_callable(
    ...
    self.amp_policy_data = ReplayBuffer(
      ...
      discriminator.observation_horizon,
    ...
    params = [
      {
        "params": self.discriminator.architecture.parameters(),
        "weight_decay": 10e-4,
        "name": "amp_trunk",
      },
      {
        "params": self.discriminator.discriminator_logits.parameters(),
        "weight_decay": 10e-2,
        "name": "amp_head",
      },
    ]
    self.discriminator_optimizer = optim.Adam(
      params, lr=discriminator_learning_rate, betas=(0.0, 0.9)
    )
  ...
  def process_env_step(
    self, obs, rewards, dones, extras, next_obs, amp_observation_buf
  ):
  ...
  def update(self):  # noqa: C901
    ...
    for sample, sample_amp_policy, sample_expert_data in zip(
      generator, amp_policy_generator, amp_expert_generator, strict=True
    ):
      (
        obs_batch,
        actions_batch,
      ...
      if self.amp_state_normalizer is not None:
        with torch.no_grad():
          ...
          policy_flat_norm = self.amp_state_normalizer.normalize_torch(
            policy_flat, self.device
          )
          expert_flat_norm = self.amp_state_normalizer.normalize_torch(
            expert_flat, self.device
          )
      ...
      if self.discriminator_loss_function == "BCEWithLogitsLoss":
        ...
        grad_pen_loss = self.discriminator.compute_wgan_grad_pen(
          expert_data, policy_data
        )
      else:
      ...
      # Sync disc grads before stepping so all ranks stay aligned.
      if self.is_multi_gpu:
    ...
    # Keep AMP normalizer stats consistent across ranks (once per update).
    if self.is_multi_gpu and self.amp_state_normalizer is not None:
      self._broadcast_amp_state_normalizer()
  ...
  def _broadcast_amp_state_normalizer(self) -> None:
    """Keep AMP observation normalizer statistics identical across ranks."""
    payload = [
      {
        "mean": self.amp_state_normalizer.mean,
        "var": self.amp_state_normalizer.var,
        "count": self.amp_state_normalizer.count,
      }
    ]
    torch.distributed.broadcast_object_list(payload, src=0)
    stats = payload[0]
    self.amp_state_normalizer.mean = stats["mean"]
    self.amp_state_normalizer.var = stats["var"]
    self.amp_state_normalizer.count = stats["count"]
    if hasattr(self.amp_state_normalizer, "_cache_dirty"):
      self.amp_state_normalizer._cache_dirty = True

  def reduce_discriminator_parameters(self) -> None:
    """Average discriminator gradients across GPUs (call before disc step)."""
    grads = [
      param.grad.view(-1)
  ...
  def reduce_parameters(self):
    ...
    Called after the PPO backward pass. Discriminator gradients are synced
    separately in ``reduce_discriminator_parameters`` before the disc step.
    """
    ...
    # Get all parameters (policy + optional RND; disc handled separately)

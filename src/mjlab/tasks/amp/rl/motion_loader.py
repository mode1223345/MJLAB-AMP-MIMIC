...
class MotionLoader:
  def __init__(
    ...
    self.KEY_POS_LOCAL_SIZE = key_pos_local_size
    self.LINEAR_VEL_SIZE = 3
    self.ANGULAR_VEL_SIZE = 3
    self.JOINT_VEL_SIZE = joint_pos_size
    self.CONTACT_MASK_SIZE = contact_mask_size
    ...
    self.KEY_POS_LOCAL_START_IDX = self.JOINT_POSE_END_IDX
    self.KEY_POS_LOCAL_END_IDX = self.KEY_POS_LOCAL_START_IDX + self.KEY_POS_LOCAL_SIZE

    self.LINEAR_VEL_START_IDX = self.KEY_POS_LOCAL_END_IDX
    ...
    self.CONTACT_MASK_START_IDX = self.JOINT_VEL_END_IDX
    self.CONTACT_MASK_END_IDX = self.CONTACT_MASK_START_IDX + self.CONTACT_MASK_SIZE
    ...
    self.observation_start_dim = self.JOINT_POSE_START_IDX
    ...
    for i, motion_file in enumerate(motion_files):
      ...
      with open(motion_file) as f:
        ...
        if i == 0:
          ...
          self.key_pos_mapping = create_index_mapping(
            motion_key_pos_names, sim_key_pos_names
          )
        ...
        for f_i in range(motion_data.shape[0]):
          ...
          motion_data[f_i, self.POS_SIZE : (self.POS_SIZE + self.ROT_SIZE)] = root_rot

        # Remove first 7 observation dimensions (root_pos, root_rot).
        self.trajectories.append(
          torch.tensor(
            motion_data[:, self.ROOT_ROT_END_IDX : self.CONTACT_MASK_END_IDX],
            dtype=torch.float32,
            device=device,
          )
        )
        self.trajectories_full.append(
          torch.tensor(
            motion_data[:, : self.CONTACT_MASK_END_IDX],
            dtype=torch.float32,
            device=device,
          )
        )
  ...
  def traj_time_sample_batch(self, traj_idxs):
    ...
    time_samples = (
      self.trajectory_lens[traj_idxs] * np.random.uniform(size=len(traj_idxs)) - subst
    )
  ...
  def get_frame_at_time_batch(self, traj_idxs, times):
    ...
    idx_low, idx_high = (
      np.floor(p * n).astype(np.int32),
      np.ceil(p * n).astype(np.int32),
    )
  ...
  def get_full_frame_at_time_batch(self, traj_idxs, times):
    ...
    all_frame_AMP_starts = torch.zeros(
      len(traj_idxs),
      self.CONTACT_MASK_END_IDX - self.JOINT_POSE_START_IDX,
      device=self.device,
    )
    ...
    for traj_idx in set(traj_idxs):
      ...
      all_frame_AMP_starts[traj_mask] = trajectory[idx_low[traj_mask]][
        :, self.JOINT_POSE_START_IDX : self.CONTACT_MASK_END_IDX
      ]
  ...
  def blend_frame_pose(self, frame0, frame1, blend):
    ...
    tar_toe_pos_0, tar_toe_pos_1 = (
      self.get_key_pos_local(frame0),
      self.get_key_pos_local(frame1),
    )
    ...
    blend_root_rot = np_quaternion_slerp(
      root_rot0.cpu().numpy(), root_rot1.cpu().numpy(), blend
    )
    blend_root_rot = torch.tensor(
      standardize_quaternion(blend_root_rot), dtype=torch.float32, device=self.device
    )
  ...
  def feed_forward_generator(self, num_mini_batch, mini_batch_size):
    for _ in range(num_mini_batch):
      ids = torch.randint(
        0, self.num_preload_transitions, (mini_batch_size,), device=self.device
      )
  ...
  def get_key_pos_local(self, pose):
    return pose[self.KEY_POS_LOCAL_START_IDX : self.KEY_POS_LOCAL_END_IDX]

  def get_key_pos_local_batch(self, poses):
    return poses[:, self.KEY_POS_LOCAL_START_IDX : self.KEY_POS_LOCAL_END_IDX]
  ...
  def get_contact_mask_batch(self, poses):
    return poses[:, self.CONTACT_MASK_START_IDX : self.CONTACT_MASK_END_IDX]


class MotionLoaderMultiAmp(MotionLoader):
  """Multi-style motion loader for AMP.

  Each motion file is treated as a style by default. A style map can group
  multiple trajectories into a shared style label.
  """
  ...
  def __init__(
    ...
    super().__init__(
      ...
      key_pos_local_size=key_pos_local_size,
      ...
      sim_key_pos_names=sim_key_pos_names,
    )

    if style_map is None:
      self.traj_style_ids = np.arange(len(self.trajectory_idxs), dtype=np.int32)
    else:
      if len(style_map) != len(self.trajectory_idxs):
        raise ValueError("style_map length must match number of trajectories")
      self.traj_style_ids = np.array(style_map, dtype=np.int32)

    self.num_styles = (
      int(self.traj_style_ids.max()) + 1 if len(self.traj_style_ids) > 0 else 0
    )

    if style_weights is None:
      if self.num_styles > 0:
        style_weights = np.zeros(self.num_styles, dtype=np.float64)
        for style_id in range(self.num_styles):
          style_weights[style_id] = np.sum(
            self.trajectory_weights[self.traj_style_ids == style_id]
          )
        style_weights = style_weights / np.sum(style_weights)
      else:
        style_weights = np.zeros(0, dtype=np.float64)
    else:
      if len(style_weights) != self.num_styles:
        raise ValueError("style_weights length must match number of styles")
      style_weights = np.array(style_weights, dtype=np.float64)
      style_weights = style_weights / np.sum(style_weights)
    self.style_weights = style_weights

    if hasattr(self, "motion_label") and self.num_styles > 0:
      traj_ids = torch.argmax(self.motion_label, dim=1).cpu().numpy()
      style_ids = self.traj_style_ids[traj_ids]
      self.motion_label = torch.nn.functional.one_hot(
        torch.tensor(style_ids, dtype=torch.long, device=self.device),
        num_classes=self.num_styles,
      ).float()
    self.style_label = getattr(self, "motion_label", None)
...
def create_index_mapping(motion_joint_names, sim_joint_names):
  if set(motion_joint_names) != set(sim_joint_names):
    only_motion = sorted(set(motion_joint_names) - set(sim_joint_names))
    only_sim = sorted(set(sim_joint_names) - set(motion_joint_names))
    raise ValueError(
      "Motion and sim joint/body name sets differ. "
      f"only_in_motion={only_motion}, only_in_sim={only_sim}"
    )

  target_name_to_index = {name: idx for idx, name in enumerate(sim_joint_names)}

  sim2motion_map = []
  for _source_idx, name in enumerate(motion_joint_names):
    target_idx = target_name_to_index[name]
    sim2motion_map.append(target_idx)

  motion2sim_map = []
  for _target_idx, name in enumerate(sim_joint_names):
...
def QuaternionNormalize(q):
  ...
  if np.isclose(q_norm, 0.0):
    raise ValueError(
      f"Quaternion may not be zero in QuaternionNormalize: |q| = {q_norm:f}, q = {q}"
    )
  return q / q_norm
